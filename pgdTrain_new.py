...
from utils import read_tar, get_model
...
class PGDtraining(BaseDefense):
    ...
    def train_all_clean(self, device, train_loader, optimizer, epoch):
...
if __name__ == '__main__':
    ...
    params = torch.load("defense_models/pgdtraind__epoch10.pth") # 从第10轮继续
    resnet50.load_state_dict(params)
    adv_loader = ImageNet.get_data_loaders("0613/resnet/adv_tar_img/", batch_size=batch_size, num_workers=2)
    pgd_adv_trainer.generate(adv_loader, test_loader, test_target_file_path)

    #test
    params = torch.load("defense_models/pgdtraind__epoch10.pth") # 从第10轮继续
    resnet50.load_state_dict(params)
    test_loader = ImageNet.get_data_loaders("dataset_train/test", batch_size=1, num_workers=2)
    #test_loader = ImageNet.get_data_loaders("dataset_train/train", batch_size=1, num_workers=2)
    test_target_file_path = "dataset_train/targets_test.txt"
    #pgd_adv_trainer.test(resnet50, "cuda", test_loader, test_target_file_path)

    label_path = "dataset_train/labels_test.txt"
    pgd_adv_trainer.test_clean(resnet50, "cuda", test_loader, label_path)
